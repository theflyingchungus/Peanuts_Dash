...
func _load_level_secret(level_number: int) -> void:
	# Fade out
	await _fade(1.0)
	...
	# Change Level
	var level_path = "res://level_scenes/level%ssecret.tscn" % level_number
	current_level_root = load(level_path).instantiate()
	add_child(current_level_root)
	current_level_root.name = "LevelRoot"
	_setup_level(current_level_root)
...
func _setup_level(level_root: Node) -> void:
	...
	# Connect Exit 2
	var exit_2 = level_root.get_node_or_null("Exit2")
	if exit_2:
		exit_2.body_entered.connect(_on_exit_2_body_entered)

	# Connect Exit Secret
	var exit_s = level_root.get_node_or_null("ExitSecret")
	if exit_s:
		exit_s.body_entered.connect(_on_exit_s_body_entered)
	...
	# Connect Entry 2
	var entry_2 = level_root.get_node_or_null("Entry2")
	if entry_2:
		entry_2.body_entered.connect(_on_entry_2_body_entered)

	# Connect Entry Secret
	var entry_s = level_root.get_node_or_null("EntrySecret")
	if entry_s:
		entry_s.body_entered.connect(_on_entry_s_body_entered)
...
func _on_exit_2_body_entered(body: Node2D) -> void:
	if body.name == "Player":
		level += 1
		body.lock_movement()
		entry_spawn_point = "FromPreviousLevel2"
		GameState.set_spawn_point(entry_spawn_point)
		call_deferred("_load_level", level)

func _on_exit_s_body_entered(body: Node2D) -> void:
	if body.name == "Player":
		body.lock_movement()
		entry_spawn_point = "FromPreviousLevel"
		GameState.set_spawn_point(entry_spawn_point)
		call_deferred("_load_level_secret", level)
...
func _on_entry_2_body_entered(body: Node2D) -> void:
	if body.name == "Player":
		level -= 1
		body.lock_movement()
		entry_spawn_point = "FromNextLevel2"
		GameState.set_spawn_point(entry_spawn_point)
		call_deferred("_load_level", level)

func _on_entry_s_body_entered(body: Node2D) -> void:
	if body.name == "Player":
		body.lock_movement()
		entry_spawn_point = "FromSecretLevel"
		GameState.set_spawn_point(entry_spawn_point)
		call_deferred("_load_level", level)
